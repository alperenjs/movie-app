...
function MovieCard({ id, title, vote_average, img_url }) {
  let img = "https://image.tmdb.org/t/p/w342/" + img_url;

  return (
    <Link to={`/movie/${id}`}>
      <div className="movie-item cursor-pointer hover:scale-110 duration-100 flex flex-col rounded-xl overflow-hidden aspect-square border dark:border-zinc-600">
        <img src={img} className=" h-4/5 object-cover w-full  " alt={title} />
        <div className="w-full h-1/5 bg-white dark:bg-zinc-800 dark:text-white px-3 flex items-center justify-between border-t-2 border-t-red-600">
          <span className="capitalize  font-medium truncate">{title}</span>
